// One-off admin script: lets a claimer position their post's image within
// the country's silhouette (drag to pan, wheel/pinch to zoom — see
// ImagePositioner.tsx) and persists that choice. Builds on
// scripts/setup-throne-system.mjs + setup-moderation.mjs + setup-beta-mode.mjs
// — extends throne_claims/payments with columns for it and threads them
// through claim_throne()/claim_throne_beta()/finalize_payment().
//
// The five new columns:
//   post_image_url                — the *chosen* photo (a post can have
//                                    several; this may not be the first one)
//   post_image_width/height       — natural pixel size of that photo,
//                                    captured client-side once so the map/
//                                    cards can reproduce the exact crop
//                                    without re-fetching the image just to
//                                    read its dimensions
//   post_image_scale              — >=1, relative to the auto "cover" fit
//   post_image_offset_x/y         — fraction of the crop box's own size
// See src/lib/image-crop.ts for the shared math all three renderers
// (positioner, map, leader-image cards) use against these values.
//
// Usage:
//   DATABASE_URL="postgresql://postgres:...@db.xxxx.supabase.co:5432/postgres" node scripts/setup-image-crop.mjs
//
...
// Existing claims simply get NULL for the four new numeric columns — every
// renderer already treats that as "use the default (cover-fit, no user
// adjustment)" the same way it always has, so nothing needs a backfill.
...
async function main() {
  ...
  await client.query(`
    alter table throne_claims
      add column if not exists post_image_width int,
      add column if not exists post_image_height int,
      add column if not exists post_image_scale numeric(6,3),
      add column if not exists post_image_offset_x numeric(6,4),
      add column if not exists post_image_offset_y numeric(6,4);
  `);
  await client.query(`
  ...
  // --- thrones_with_leader / throne_claims_public: append the new columns
  // (create-or-replace-view can only append, never reorder/remove — same
  // constraint noted in setup-moderation.mjs).
  await client.query(`
  ...
  // --- claim_throne(): same signature plus the five image params. Adding
  // params changes the signature, so drop the old exact one first (same
  // reasoning as setup-moderation.mjs's own claim_throne change).
  await client.query(`
    drop function if exists claim_throne(text, text, numeric, jsonb, text, text, text, text, timestamptz, inet, text, text, text, text);
  `);
  await client.query(`
    create or replace function claim_throne(
      ...
      p_post_image_width int default null,
      p_post_image_height int default null,
      p_post_image_scale numeric default null,
      p_post_image_offset_x numeric default null,
      p_post_image_offset_y numeric default null
    ) returns bigint
    language plpgsql
    as $$
    declare
  ...
  // --- claim_throne_beta(): same treatment.
  await client.query(`
    drop function if exists claim_throne_beta(text, text, jsonb, text, text, text, text, timestamptz, inet, text, text, text, text, text);
  `);
  await client.query(`
  ...
  // --- finalize_payment(): own signature is unchanged, just needs to pass
  // the five new payments columns through to claim_throne().
  await client.query(`
  ...
  console.log("Image cropping set up: throne_claims/payments columns, updated views, claim_throne(), claim_throne_beta(), finalize_payment().");
